...
export class NodeTransport {
  ...
  constructor(public device: SerialPort, usbVendorId?: number, usbProductId?: number) {
  ...
  async write(data: Uint8Array) {
    ...
    if (this.device.writable) {
      ...
      await new Promise((resolve, reject) => {
        this.device.drain((err: Error | null) => {
          if (err) {
            reject(err);
          } else {
            resolve(null);
    ...
    try {
      ...
      do {
        ...
        if (done) {
          ...
          await new Promise((resolve, reject) => {
            this.device.flush((err: Error | null) => {
              if (err) {
                reject(err);
              } else {
                resolve(null);
  ...
  async connect(baud = 115200) {
    ...
    await new Promise((resolve, reject) => {
      this.device.update({ baudRate: baud }, (err: Error | null) => {
        if (err) {
          reject(err);
        } else {
          resolve(null);
  ...
  async disconnect() {
    await new Promise((resolve, reject) => {
      this.device.close((err: Error | null) => {
        if (err) {
          reject(err);
        } else {
          resolve(null);
